...
def clear_voice_assistent():
    clear_console()
    print_center_zeile("Choose one of your own created creative functions and have fun!", 2, "center")               
    print_center_zeile("****************************************************************", 3, "center")
...
def call_print_function_thread(debug_mode):
    ...
    print_functions_thread = threading.Thread(target=print_functions, daemon=True, args=(functions, debug_mode,))
...
def print_functions(functions, debug_mode,): 
    ...
    while not stop_flag: 
        ...
        if function_description is not None and len(function_description) >= 3 and stop_flag == False:
            text_1 = show_functions.colorize_signal_words(sentence_start, sentence_end, word, and_word, function_description, color=show_functions.ConsoleColors.GREEN)
            anzeigen_und_ausblenden(text_1, speed=4)
...
def anzeigen_und_ausblenden(text_1, speed=5):
    global stop_flag
...
#Waiting_Jokes Anfang
def stop_waiting_jokes_thread():
    global stop_flag
    global waiting_jokes_thread
    if waiting_jokes_thread.is_alive():
        stop_flag = True
        if debug_mode == True:print("Stopping waiting_jokes_thread...")
        waiting_jokes_thread.join()  # Wartet darauf, dass der Thread beendet wird
        if debug_mode == True:print("waiting_jokes_thread stopped.")            
    else:
        if debug_mode == True:print("waiting_jokes_thread is not running.")       
        

def call_waiting_jokes_thread(debug_mode):
    global waiting_jokes_thread
    functions = function_collection(0)    
    waiting_jokes_thread = threading.Thread(target=print_waiting_jokes, daemon=True, args=(debug_mode,))
    waiting_jokes_thread.start()

def print_waiting_jokes(debug_mode): 
    global stop_flag
    stop_flag = False
    while not stop_flag: 
        text_1 = get_random_sentence_from_file(file_path_satz, "wartesätze")
        
        anzeigen_und_ausblenden_waiting_jokes(text_1, speed=5)
...
def anzeigen_und_ausblenden_waiting_jokes(text_1, speed=5):
    global stop_flag
    terminal_breite, terminal_hoehe = shutil.get_terminal_size()
    ...
    while True:
        ...
        while True:
            ...
            if input_text:
                ...
                if debug_mode == False:
                    print_center_zeile(f"Erkannter Text: {input_text}", 1, "left")  # Füge diese Debugging-Ausgabe hinzu
...
def ask_for_translation(lang, antwort):
    ...
    if lang == "en":
        translation = hugchat("Bitte übersetze mir folgendes auf deutsch: " + antwort + " Die Antwort soll kein englisches Wort enthalten und nur den übersetzten Text enthalten")
        return translation
    else:
        translation = antwort
        return translation
...
def hugchat(user_input):
    ...
    if chatbot is None:
        ...
        antwort = "Ich konnte keine Antwort von HugChat erhalten."
        return antwort

    if debug_mode == True: print(f"Versuche mit HugChat zu chatten: {user_input}")

    # Nachricht eingeben
    antwort = chatbot.chat(user_input, conversation=conversation)
    if debug_mode == True: print(f"Antwort: {antwort}")
    ...
    if debug_mode == True: print(antwort_text)
...
def huggingfacechat(engine, user_input, chatbot, pyttsx_voice_id, conn, section_name):
    ...
    antwort = hugchat(user_input)
    dialogmodus_flag= True
    ...
    if antwort == "Ich konnte keine Antwort von HugChat erhalten.":
        stop_waiting_jokes_thread()
        clear_console()
        dialogmodus_flag = False
        return dialogmodus_flag
            ...
            for i in range(1, 16):
                    ...
                    if contains_signal_word(user_input, signal_words, and_signal_words, section_name):
                        ...
                        threading.Thread(target=ultimate_light, daemon=True, args=(
                                                    tapo_section_name, anschalten_lamp_color, anschalten_lamp_temp, ausschalten_lamp, anschalten_lamp, anschalten_plug, ausschalten_plug, lamp_plug_dict,
                                                    startfarbe_d1, brightness_d1, startfarbe_d2, brightness_d2, startfarbe_d3, brightness_d3, startfarbe_d4, brightness_d4,
...
def tapo_contains_signal_word(user_input, tapo_signal_words, tapo_and_signal_words, tapo_section_name):
    try:
        ...
        if len(tapo_and_signal_words) > 0:
            and_bedingung_erfuellt = all(word in normalized_input for word in tapo_and_signal_words)
        else:
            and_bedingung_erfuellt = True  # 'and'-Bedingung ist erfüllt, wenn keine Wörter angegeben sind
        if debug_mode == True: print("and_bedingung_erfuellt: ")
        if debug_mode == True: print(and_bedingung_erfuellt)
...
def vosk_sprachepaket_laden(vosk_sprache_deutsch, vosk_sprache_englisch, vosk_sprache_spanisch):
